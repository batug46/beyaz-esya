        ...
        <section className="py-24 sm:py-32 relative bg-[var(--background)] overflow-hidden transition-colors duration-500">
            ...
            <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                <div className="text-center mb-16 sm:mb-24">
                    <span className="text-accent text-[10px] sm:text-xs font-black uppercase tracking-[0.4em] mb-4 block">Geleceğin Standartları</span>
                    <h3 className="text-3xl sm:text-5xl lg:text-7xl font-black text-[var(--text-main)] mb-8 tracking-tighter uppercase break-words px-2">
                        Neden <span className="text-accent">Techhome?</span>
                    </h3>
                    <p className="text-[var(--text-dim)] text-base sm:text-lg font-medium max-w-2xl mx-auto leading-relaxed">
                    ...
                    {items.map((item, idx) => (
                        <div key={idx} className={item.featured ? "xl:col-span-2 group" : "group"}>
                            <div className={`premium-card p-10 rounded-[40px] h-full flex flex-col ${item.featured ? "justify-center relative overflow-hidden" : "items-center justify-center text-center"}`}>
                                ...
                                <div className={`${item.featured ? "w-16 h-16 sm:w-20 sm:h-20" : "w-14 h-14 sm:w-16 sm:h-16 bg-[var(--surface)] border border-[var(--glass-border)]"} bg-accent rounded-3xl flex items-center justify-center mb-10 shadow-[0_0_30px_rgba(45,49,250,0.3)] mx-auto ${item.featured ? "md:mx-0" : "mx-auto"}`}>
                                    {item.icon}
                                </div>
                                <h4 className={`${item.featured ? "text-2xl sm:text-4xl" : "text-xl sm:text-2xl"} font-black text-[var(--text-main)] mb-6 uppercase tracking-tighter`}>
                                    {item.title} {item.span && <span className="text-accent">{item.span}</span>}
                                </h4>
                                <p className={`text-[var(--text-dim)] text-base sm:text-lg font-medium leading-relaxed max-w-sm mx-auto ${item.featured ? 'md:mx-0' : 'md:mx-auto'}`}>
                                    {item.desc}
                                </p>
                            </div>
                        </div>
